...
CREATE TYPE public.app_role AS ENUM ('student', 'office', 'admin');
...
GRANT SELECT ON public.office_departments TO authenticated;
GRANT ALL ON public.office_departments TO service_role;
...
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT EXISTS (SELECT 1 FROM public.office_departments WHERE user_id = _user_id AND department_id = _department_id)
$$;

CREATE POLICY "office read own assignments" ON public.office_departments FOR SELECT TO authenticated
  USING (user_id = auth.uid());
CREATE POLICY "admins manage assignments" ON public.office_departments FOR ALL TO authenticated
  USING (public.has_role(auth.uid(), 'admin')) WITH CHECK (public.has_role(auth.uid(), 'admin'));

CREATE POLICY "own profile readable" ON public.profiles FOR SELECT TO authenticated
  USING (id = auth.uid() OR public.has_role(auth.uid(), 'admin') OR public.has_role(auth.uid(), 'office'));
...
CREATE POLICY "students read own application" ON public.clearance_applications FOR SELECT TO authenticated
  USING (student_id = auth.uid() OR public.has_role(auth.uid(), 'office') OR public.has_role(auth.uid(), 'admin'));
...
CREATE POLICY "office and admin update reviews" ON public.department_reviews FOR UPDATE TO authenticated
...
CREATE POLICY "office read clearance docs" ON storage.objects FOR SELECT TO authenticated
  USING (bucket_id = 'clearance-docs' AND (public.has_role(auth.uid(), 'office') OR public.has_role(auth.uid(), 'admin')));
...
BEGIN
  ...
  FROM (
    SELECT sd.user_id
    FROM public.office_departments sd
